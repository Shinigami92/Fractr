...
function snapshotCameraPos(camera: FPSCamera): CameraPos {
  return {
    x: camera.position[0]!,
    y: camera.position[1]!,
    z: camera.position[2]!,
...
function resetCameraFromConfig(camera: FPSCamera, fractal: FractalParamsStore): void {
 ...
 */
function buildLiveSceneParamsFor(
  camera: FPSCamera,
  fractal: FractalParamsStore,
  graphics: GraphicsSettingsStore,
  overrides?: LiveSceneParamOverrides,
): LiveSceneParams {
  return {
    power: fractal.power,
    ...
    originOffset: computeOriginOffset(camera, fractal),
  };
}
...
export function useSceneState(): UseSceneStateReturn {
  ...
  const cameraPos = ref<CameraPos>({ x: 0, y: 0, z: 3, yaw: 0, pitch: 0, roll: 0 });

  return {
    camera,
    cameraPos,
    syncCameraPos: () => {
      cameraPos.value = snapshotCameraPos(camera);
    },
    resetCamera: () => {
      resetCameraFromConfig(camera, fractal);
    },
    getCurrentState: () => snapshotSavedState(camera, fractal, graphics),
    applyState: (state) => {
      applySavedState(camera, fractal, graphics, state);
    },
    buildLiveSceneParams: (overrides) =>
      buildLiveSceneParamsFor(camera, fractal, graphics, overrides),
  };
}
